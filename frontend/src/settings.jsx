...
import { Settings, Save, Eye, EyeOff, X, Loader2, TestTube, CheckCircle, XCircle, Mic, Shield, Radio } from "lucide-react";
    ...
    try {
      ...
      if (window.electronAPI) {
        ...
        const loadedSettings = {
          ...
          asr_mode: allSettings.asr_mode || "local",
          openai_realtime_api_key: allSettings.openai_realtime_api_key || "",
        };
  ...
  const saveSettings = async () => {
    try {
      ...
      if (window.electronAPI) {
        ...
        await window.electronAPI.setSetting('asr_mode', settings.asr_mode);
        await window.electronAPI.setSetting('openai_realtime_api_key', settings.openai_realtime_api_key);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mb-6">
            <div className="p-6">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                  选择语音转文字引擎。本地模式使用 voxtral 离线识别，云端模式使用 OpenAI Realtime API。
                </p>
              </div>

              <div className="space-y-3">
                <label className="flex items-start space-x-3 p-3 rounded-lg border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <input
                    type="radio"
                    name="asr_mode"
                    value="local"
                    checked={settings.asr_mode === 'local'}
                    onChange={() => handleInputChange('asr_mode', 'local')}
                    className="mt-0.5 w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">本地 Voxtral</span>
                ...
                <label className="flex items-start space-x-3 p-3 rounded-lg border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  <input
                    ...
                    name="asr_mode"
                    value="cloud"
                    checked={settings.asr_mode === 'cloud'}
                    onChange={() => handleInputChange('asr_mode', 'cloud')}
                    className="mt-0.5 w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                  />
                  <div>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">云端 OpenAI Realtime</span>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      使用 OpenAI Realtime API，需要 API Key 和网络连接
                    </p>
                  </div>
                </label>
                ...
                {settings.asr_mode === 'cloud' && (
                  <div className="ml-7 mt-2">
                    ...
                    <input
                      type="password"
                      value={settings.openai_realtime_api_key}
                      onChange={(e) => handleInputChange('openai_realtime_api_key', e.target.value)}
                      placeholder="sk-..."
                      className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      用于连接 OpenAI Realtime API 的密钥（与上方 AI 文本优化的 Key 独立）
                    </p>
                  </div>
                )}
              </div>

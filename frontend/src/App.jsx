...
export default function App() {
    ...
    const loadAsrSettings = async () => {
      try {
        if (window.electronAPI) {
          ...
          const mode = allSettings.asr_mode || 'local';
          setAsrMode(mode);
          if (mode === ASR_MODES.CLOUD) {
            setAsrApiKey(allSettings.openai_realtime_api_key || '');
          }
        }
      } catch (error) {
        console.error('加载 ASR 设置失败:', error);
      }
    };
    loadAsrSettings();
  }, []);
  ...
  const handleRecordingComplete = useCallback(async (transcriptionResult) => {
    ...
    if (transcriptionResult.success && transcriptionResult.text) {
      ...
      // 注意：不在这里保存到数据库，由录音链路统一处理保存逻辑

      toast.success("🎤 实时识别完成，AI正在优化文本...");
    } else {
  ...
  }, [asrMode, modelStatus, isRecording, isRecordingProcessing, startRecording, stopRecording, cancelRecording]);
  ...
  // Realtime 流式结果直接更新到主文本区，保证按下热键后可见
  useEffect(() => {
    if (!partialText) return;
    setOriginalText(partialText);
    setShowTextArea(true);
  }, [partialText]);
  ...
  const getMicButtonProps = () => {
    ...
    // 如果模型未就绪（仅本地模式），显示禁用状态
    if (asrMode !== ASR_MODES.CLOUD && !modelStatus.isReady) {
      return {
      ...
      <div className="max-w-2xl mx-auto min-h-screen flex flex-col">
        ...
        <div className="text-center mb-8 flex-shrink-0">
          <Tooltip content={micProps.tooltip}>
            ...
            >
              {/* 动态内容基于状态 */}
              {asrMode !== ASR_MODES.CLOUD && modelStatus.stage === 'downloading' ? (
                <LoadingIndicator size={20} />
              ) : asrMode !== ASR_MODES.CLOUD && (modelStatus.stage === 'loading' || !modelStatus.isReady) ? (
                <LoadingIndicator size={20} />
              ) : micState === "idle" ? (
          ...
          <p className="mt-4 status-text text-gray-700 dark:text-gray-300">
            {asrMode === ASR_MODES.CLOUD ? (
              micState === "recording" ? (
                ...
                `当前模式：云端 OpenAI，点击麦克风或按 ${hotkey} 开始录音`
              )
              ...
              `当前模式：实时识别，点击麦克风或按 ${hotkey} 开始录音`
            )}
          </p>
        </div>

        {/* 模型下载进度显示（仅本地模式） */}
        {asrMode !== ASR_MODES.CLOUD && (modelStatus.stage === 'need_download' || modelStatus.stage === 'downloading') && (
          <div className="mb-6">
